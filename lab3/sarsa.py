...
while penalities > 0:
    ...
    j = randint(0,n-1)
    if reward[i][j]==0 and (i,j)!=(0,0) and (i,j)!=(n-1,n-1):
        if (i==0 and j==2) or (i==1 and j==0) or (i==2 and j==2) or (i==3 and j==1):
            reward[i][j] = -1
            penalities -= 1
            colors[i][j] = (255,0,0)
            terminals.append((i,j))
...
def step():
    ...
    if new_pos not in terminals:
        #此处需要实现sarsa算法
        action_pie = select_action(new_pos)
        qTarget = reward[new_pos] + gamma * Q[tuple(new_pos)][action_pie]

    else:
        #此处需要实现sarsa算法
        qTarget = reward[new_pos[0], new_pos[1]]
        new_pos = (0, 0)
        action_pie = -1
    Q[tuple(current_pos)][action] += alpha * (qTarget - Q[tuple(current_pos)][action])
