...
def train(model, target_model, start, path, C=10):
    ...
    while iteration < model.number_of_iterations:
        ...
        if terminal:
            ...
            #     update_nums = 0
            episode += 1
            r_sum = reward_sum.detach().item()
            timeStep = iteration - last_iteration
            last_iteration = iteration
            reward_episode = [episode, timeStep, r_sum]
            print("episode=",episode, "iterations=", timeStep, 'reward=', r_sum)
...
def main(mode):
    ...
    if mode == 'test':
        ...
        start = time.time()
        train(model, target_model, start, path="./C=30/record.csv", C=30)
