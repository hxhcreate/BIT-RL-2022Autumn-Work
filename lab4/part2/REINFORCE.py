...
POLICY_TRAIN_ITERS = 2  # Number of iterations of policy improvement in each epoch
...
def train(S,A,returns):
    ...
    for i in range(POLICY_TRAIN_ITERS):
        ...
        ####填空部分，一行，填写REINFORCE的
        # loss函数，objective = #######
        objective = - torch.mean(log_probs * returns * (GAMMA ** n))
        objective.backward()
        OPT.step()
...
plt.savefig("images/reinforce-"+args.mode+"-3.png")
