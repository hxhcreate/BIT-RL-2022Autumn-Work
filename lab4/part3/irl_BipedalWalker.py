...
class GAIL:
    ...
    def train(self, n_iter):
        ...
        for train_step in range(n_iter+1):
            ...
            loss_discriminator = self.loss_fn(self.discriminator(exp_state, exp_action), 
                                              torch.ones_like(self.discriminator(exp_state, exp_action)))
            loss_discriminator += self.loss_fn(self.discriminator(state.detach(), action.detach()),
                               torch.zeros_like(self.discriminator(state.detach(), action.detach())))
            ...
            # 以下是填空部分，以下两行需要用实际的loss和gradient step来替换，现在的这两行是占位符来确保程序可编译
            loss_actor = self.loss_fn(self.discriminator(state, action),
                                      torch.ones_like(self.discriminator(state, action)))
            loss_actor.backward()
...
class BehaviorCloning:
    ...
    def train(self, n_iter):
        ...
        for train_step in (range(n_iter + 1)):
            ...
            #填写BC的loss函数(一行)，训练模型
            # output as input, exp_action as target, use MSELossFunction
